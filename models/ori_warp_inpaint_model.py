...
class WarpInpaintModel(torch.nn.Module):
    def __init__(self, config):
        ...
        img_path = config["img_path"] if "img_path" in config else "samples/motor1.png"
        image = Image.open(img_path).convert('RGB').resize((512, 512))
        print(f'loaded image from {img_path}')
        self.image_tensor = ToTensor()(image).unsqueeze(0).to(self.device)

        self.depth_model = torch.hub.load("MiDaS", "DPT_Large", path="/mnt/storage/user/wangxiaodong/.cache/torch/hub/checkpoints/dpt_large_384.pt", source="local").to(self.device)
    ...
    def get_next_camera_circle(self):
        ...
        x, y, z = 0.3, -0.3, -0.3  # initial values
