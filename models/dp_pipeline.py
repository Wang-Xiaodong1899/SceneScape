...
from models.mesh_renderer_pipeline import Renderer
...
class WarpInpaintModel(torch.nn.Module):
    def __init__(self, config, image_path, prompt, extrinsics, intrinsics):
        ...
        self.video_name = image_path.split('/')[-2]
        ...
        if self.config["use_splatting"]:
            ...
            self.renderer = Renderer(config, image_size=512, fl1=self.intrinsics[0][0][0], fl2=self.intrinsics[0][1][1], antialiasing_factor=aa_factor)
    ...
    def update_mesh(self, image, depth, mask, extrinsic, epoch):
        ...
        # scale = 5
        updated_depth = updated_depth * 40
